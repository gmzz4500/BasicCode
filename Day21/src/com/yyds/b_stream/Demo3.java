...
public class Demo3 {
    public static void main(String[] args) {
        ...
        //把上一步操作后的元素作为构造方法的参数创建演员对象,遍历数据
        Stream<Actor> actorStream = Stream.concat(manList.stream().filter(name->name.length() == 3).limit(3), womanList.stream().filter(name -> name.startsWith("林")).skip(1)).map(Actor::new);
